...
def make_env(templates_dir: Path) -> Environment:
    return Environment(loader=FileSystemLoader(str(templates_dir)), autoescape=select_autoescape(["html", "xml"]), trim_blocks=True, lstrip_blocks=True)
...
def render_weekly_pages(weekly_issues: list[dict[str, Any]], templates_dir: Path, out_dir: Path, config_path: Path | None) -> list[Path]:
    ...
    for weekly in weekly_issues:
        written.append(render_weekly(Path(str(weekly["source_path"])), templates_dir, weekly_out_dir, config_path))
    return written


def render_weekly_index(weekly_issues: list[dict[str, Any]], templates_dir: Path, out_dir: Path, config: dict[str, Any]) -> Path:
    template = make_env(templates_dir).get_template("weekly_index.html.j2")
    weekly_dir = out_dir / "weekly"
    weekly_dir.mkdir(parents=True, exist_ok=True)
    path = weekly_dir / "index.html"
    path.write_text(template.render(config=config, weekly_issues=weekly_issues, latest=weekly_issues[0] if weekly_issues else None, page_url=absolute_url(str(config.get("base_url", "")), "weekly/"), asset_prefix="../", trust_nav_links=TRUST_NAV_LINKS, analytics_snippet=analytics_snippet(config), og_image_url=absolute_url(str(config.get("base_url", "")), "static/og-image.png")), encoding="utf-8")
    return path
...
def ensure_home_weekly_nav(out_dir: Path) -> Path | None:
    ...
    if 'href="weekly/"' not in html:
        if 'href="archive.html"' in html:
            html = html.replace('href="archive.html"', 'href="weekly/">Weekly</a> <a href="archive.html"', 1)
        elif '</nav>' in html:
            html = html.replace('</nav>', '<a href="weekly/">Weekly</a></nav>', 1)
...
def weekly_archive_block(latest: dict[str, Any]) -> str:
    ...
    return f'''\n<section class="weekly-archive-card" aria-label="Weekly Magazine">\n  <p class="weekly-home-eyebrow">Weekly Magazine</p>\n  <h2>Open Source Signal Weekly</h2>\n  <p>Sunday editorial synthesis, separate from the Daily Signal stream.</p>\n  <p class="weekly-home-actions">\n    <a href="weekly/{output_name}">Latest weekly: {issue_number} · {week_label}</a>\n    <a href="weekly/">All weekly issues →</a>\n  </p>\n</section>\n'''


def weekly_archive_css() -> str:
    return '''\n.weekly-archive-card { width: min(980px, 100%); margin: 28px auto; padding: 22px; border: 1px solid var(--line, rgba(20,20,20,.16)); border-radius: 24px; background: rgba(255,249,234,.58); text-align: center; }\n.weekly-home-eyebrow { display: inline-flex; margin: 0 0 10px; color: var(--muted, #6d665a); font-family: var(--label-font, Arimo, Arial, sans-serif); font-size: 12px; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; }\n.weekly-home-actions { display: flex; justify-content: center; gap: 10px 14px; flex-wrap: wrap; margin-top: 16px; }\n.weekly-home-actions a { font-weight: 800; text-underline-offset: .22em; }\n'''
...
def update_sitemap(out_dir: Path, config: dict[str, Any], weekly_issues: list[dict[str, Any]]) -> Path:
    ...
    sitemap = remove_old_weekly_entries_from_sitemap(sitemap_path.read_text(encoding="utf-8"))
    ...
    for weekly in weekly_issues:
        ...
        entries.append("  <url>\n" f"    <loc>{escape(absolute_url(base_url, href))}</loc>" f"{lastmod_line}\n" "    <changefreq>weekly</changefreq>\n" "  </url>")
    insert = "\n" + "\n".join(entries) + "\n"
    sitemap = sitemap.replace("</urlset>", insert + "</urlset>", 1) if "</urlset>" in sitemap else sitemap + insert
...
def main() -> None:
    ...
    parser.add_argument("--weekly", type=Path, default=Path("weekly"))
