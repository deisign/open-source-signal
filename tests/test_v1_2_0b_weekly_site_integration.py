...
def build_full_site(out_dir: Path) -> None:
    ...
    subprocess.run([sys.executable, str(ROOT / "build_weekly_site.py"), "--weekly", str(ROOT / "weekly"), "--templates", str(ROOT / "templates"), "--out", str(out_dir), "--config", str(ROOT / "site.json")], check=True, cwd=ROOT)
...
def test_weekly_section_builds_index_issue_archive_and_sitemap(tmp_path: Path) -> None:
    ...
    assert "Sunday editorial synthesis of the strongest OSINT signals of the week" not in home.get_text(" ")
